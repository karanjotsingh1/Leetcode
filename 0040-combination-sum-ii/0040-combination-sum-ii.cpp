class Solution {
public:
    void combo_lbbo(vector<int>& candidates, int target,int idx,int &n,vector<int>&combo,vector<vector<int>>&combos)
    {
        if(target==0)
        {
        ...
        combo_lbbo(candidates,target-candidates[idx],idx+1,n,combo,combos);
        combo.pop_back();
        ...
        combo_lbbo(candidates,target,idx+1,n,combo,combos);
    ...
    vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
        ...
        combo_lbbo(candidates,target,idx,n,combo,ans);
        return ans;
    }
};
