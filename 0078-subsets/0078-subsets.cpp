class Solution {
public:
    void subsets_kddo(vector<int>& nums,int idx,int &n,vector<int>&subset,vector<vector<int>>&subsets)
    {
        if(idx>n)
            return;
        ...
        subsets_kddo(nums,idx+1,n,subset,subsets);
        subset.pop_back();
    ...
    vector<vector<int>> subsets(vector<int>& nums) {
        ...
        subsets_kddo(nums,idx,n,subset,subsets);
        return subsets;
    }
};
