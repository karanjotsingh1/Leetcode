class Solution {
public:
    void find_combo(vector<int>& candidates,int idx,int &n, int target,vector<int>&combo,vector<vector<int>>&combos)
    {
        if(target==0)
        {
        ...
        find_combo(candidates,idx,n,target-candidates[idx],combo,combos);
        combo.pop_back();

        find_combo(candidates,idx+1,n,target,combo,combos);
    ...
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        ...
        find_combo(candidates,idx,n,target,combo,combos);
        return combos;
    }
};
